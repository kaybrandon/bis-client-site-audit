...
public class AuditService(ApplicationDbContext db, PhotoService photos)
{
    ...
    public async Task<ClientAudit?> GetHeaderAsync(Guid id, CancellationToken ct = default) =>
        await db.Audits.AsNoTracking().FirstOrDefaultAsync(a => a.Id == id, ct);
    ...
    public async Task<ClientAudit> DuplicateAsync(Guid id, CancellationToken ct = default)
    {
        ...
        await CloneItems(source.Workstations, copy.Id, (s, n) => CloneWorkstation(s, n), PhotoOwnerTypes.Workstation, ct);
        await CloneItems(source.NetworkItems, copy.Id, (s, n) => CloneNetwork(s, n), PhotoOwnerTypes.Network, ct);
        await CloneItems(source.ServerStorageItems, copy.Id, (s, n) => CloneServer(s, n), PhotoOwnerTypes.ServerStorage, ct);
        await CloneItems(source.SecurityAvItems, copy.Id, (s, n) => CloneSecurity(s, n), PhotoOwnerTypes.SecurityAv, ct);
        await CloneItems(source.SoftwareItems, copy.Id, (s, n) => CloneSoftware(s, n), PhotoOwnerTypes.Software, ct);
        await CloneItems(source.Issues, copy.Id, (s, n) => CloneIssue(s, n), PhotoOwnerTypes.Issue, ct);
        await CloneItems(source.Purchases, copy.Id, (s, n) => ClonePurchase(s, n), PhotoOwnerTypes.Purchase, ct);
    ...
    private async Task CloneItems<T>(IEnumerable<T> source, Guid newAuditId, Func<T, Guid, T> clone, string ownerType, CancellationToken ct)
